<?php
class Nominal_Object extends ArrayObject
{
  ...
  public function loadFromArray($values)
  {
    foreach ($values as $k => $v) {
      if (is_array($v)) {
        $v = Nominal_Util::convertToNominalObject($v);
      }
      if (strpos(get_class($this), "Nominal_Object") !== false) {
        $this[$k] = $v;
      } else {
        $this->$k = $v;
      }
  ...
  protected function _toArray() {
    ...
    foreach ($this->_values as $k => $v) {
      if (is_object($v) == true && strpos(get_class($v), "Nominal_") !== false) {
